...
func set_hp(hp):
	...
	$Tween.interpolate_property(
		$HP/Fill, "rect_scale", scale, Vector2(hp / maxHp, 1), 1, Tween.TRANS_CIRC, Tween.EASE_OUT
	)
	$Tween.start()
...
func set_mp(mp):
	...
	$Tween.interpolate_property(
		$MP/Fill, "rect_scale", scale, Vector2(mp / maxMp, 1), 1, Tween.TRANS_CIRC, Tween.EASE_OUT
	)
	$Tween.start()
